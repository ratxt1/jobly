import React, { Component } from 'react';
import {Link} from 'react-router-dom';

const DEFAULT_IMG = "data:image/png;base64,";

class CompanyCard extends Component{
  constructor(props){
    super(props);
    this.handleClick = this.handleClick.bind(this);
  }

  handleClick(evt){
    // TODO: NEED TO GET HISTORY FROM RTPROPS
    this.props.history.push(`/companies/${this.props.handle}`);
  }


    render(){
        let { handle, name, description, logo_url } = this.props;
        if (logo_url === ""){
          logo_url = DEFAULT_IMG;
        }

        return(
          // <Link to={`/companies/${handle}`}>
          //   <span className="card" style={{"display": "block"}}>
              <div onClick={this.handleClick} className="card mx-auto mb-1" style={{width: '45rem'}}>
                <div className="card-body">
                  <h5 className="card-title">{ name }</h5>
                  <p className="card-text">{ description }</p>
                </div>
              </div> 
          //   </span>
          // </Link> 
        )
      }
}

export default CompanyCard;


/** onClick - this.props.history.push({`/companies/${handle}`}) */
